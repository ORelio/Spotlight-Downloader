...
namespace SpotlightDownloader
{
    ...
    class SpotlightImage
    {
        ...
        public string DownloadToFile(string outputDir, bool integrityCheck = true, bool metadata = false, string outputName = null)
        {
            ...
            if (integrityCheck)
            {
                if (new FileInfo(outputFile).Length != FileSize)
                {
                    ...
                    throw new InvalidDataException("SpotlightImage: File returned by server does not have the expected size.");
                }
                using (FileStream fileStream = File.OpenRead(outputFile))
                {
                    ...
                    {
                        File.Delete(outputFile);
                        throw new InvalidDataException("SpotlightImage: File returned by server does not have the expected sha256 hash.");
                    }
                }
            }
